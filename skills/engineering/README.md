...
- **[commit](./commit/SKILL.md)** — Create a Conventional Commits-style git commit from the current changes. Stages the intended files, infers type and scope from recent history, and never pushes.
- **[security](./security/SKILL.md)** — Add 7-day package-age guardrails before installs, then review for secrets, injection, authz gaps, and dependency risk.
