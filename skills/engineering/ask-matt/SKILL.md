...
- **`/commit`** — turn the current working tree into one or more Conventional Commits-style commits. It infers type and scope from recent history, asks about unrelated files, and never pushes. Reach for it when the change is already written and you only need the commit step — not a build loop.
