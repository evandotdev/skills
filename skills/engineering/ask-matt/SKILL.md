...
3. **Branch — is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally — one red-green slice at a time — then closes out by running **`/code-review`**, a two-axis review (Standards + Spec) of the diff, before committing. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point, and **`/commit`** when the work is already written and you only want a Conventional Commits-style commit.
...
- **`/commit`** — turn the current working tree into one or more Conventional Commits-style commits. It infers type and scope from recent history, asks about unrelated files, and never pushes. If `.git/hooks` is only samples, it stops and asks you to install hooks (or run a setup skill) before committing. Reach for it when the change is already written and you only need the commit step — not a build loop.
- **`/security`** — quarantine packages newer than 7 days, then sweep for secrets, injection, authz gaps, and dependency risk. Reach for it before an install, at project start, or when you want a hardening pass rather than a diff review.
